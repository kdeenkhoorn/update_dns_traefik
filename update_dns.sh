...
_get() {
  ...
  HEADER="Authorization: Basic $(printf "%s" "${CPANELDNS_AUTH_ID}:${CPANELDNS_AUTH_PASSWORD}" | base64)"
  _debug "header ${HEADER}"
  wget -q --user-agent="update_dns.sh/v1.0" --header "${HEADER}" -O - "$url"
  ret=$?
  if [ "$ret" = "8" ]; then
    ret=0
...
case "${OPTION}" in
     present)
          dns_cpaneldns_add "${HOST}" "${CHALLENGE}"
          sleep 120
          ;;
     cleanup)
          dns_cpaneldns_rm "${HOST}" "${CHALLENGE}"
          ;;
     timeout)
